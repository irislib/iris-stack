...
test('the capability table navigates within the guide', () => {
  ...
  assert.match(guide, /files as blobs—which can be split into chunks—and directories as manifest\s+trees/);
...
test('the site directly renders the repository-owned Markdown guide', () => {
  ...
  assert.match(index, /<title>Iris Stack<\/title>/);
  assert.match(index, /<meta property="og:title" content="Iris Stack" \/>/);
  assert.match(index, /<meta property="og:image" content="https:\/\/stack\.iris\.to\/social-preview\.png" \/>/);
...
test('the public guide keeps architecture prose and private operations out', () => {
  ...
  assert.match(guide, /^## 5\. Private messaging$/m);
  assert.match(guide, /nostr-double-ratchet/);
  assert.match(guide, /pairwise-ratchet-first/);
  assert.match(guide, /derive their own root by adding or removing records while reusing unchanged\s+blocks/);
  assert.match(guide, /snapshot remains readable even if its maintainer goes offline/);
